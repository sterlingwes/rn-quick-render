...
class LayoutlibTextMeasurerTest {
    ...
    fun measureSingleLineText() {
        val (width, height) = measurer.measure("Hello World", 14f, null, null, Float.MAX_VALUE)
    ...
    fun measureWrappedText() {
        ...
        val (wideW, wideH) = measurer.measure(text, 14f, null, null, Float.MAX_VALUE)
        val (narrowW, narrowH) = measurer.measure(text, 14f, null, null, 100f)
    ...
    fun boldTextMeasuresDifferently() {
        ...
        val (normalW, _) = measurer.measure(text, 14f, null, null, Float.MAX_VALUE)
        val (boldW, _) = measurer.measure(text, 14f, "bold", null, Float.MAX_VALUE)
    ...
    fun fontSizeAffectsOutput() {
        val text = "Scale test"
        val (smallW, smallH) = measurer.measure(text, 12f, null, null, Float.MAX_VALUE)
        val (largeW, largeH) = measurer.measure(text, 24f, null, null, Float.MAX_VALUE)
    ...
    fun emptyTextReturnsZero() {
        val (w, h) = measurer.measure("", 14f, null, null, Float.MAX_VALUE)
